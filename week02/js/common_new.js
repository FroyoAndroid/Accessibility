...
function ittloadpage(){
	$.ajax({
		...
		complete:function(){
			...
			//alert(parseFloat(navigator.appVersion.split("MSIE")[1]));
			if (navigator.userAgent.match(/msie/i) && parseFloat(navigator.appVersion.split("MSIE")[1]) <= 9 ) {
			
        alert ("It looks like you are using Internet Explorer. To be able to access this course, you need to upgrade to version 10 and above.")
    }
...
function sscResizeWindow(){
	...
	$('.book_title').css('width',parseInt($('.sscBranding').width())-140 +'px');
...
function loadListItems(){
   ...
   // $('#currentSlide').html(currSlide + " of " + menuCount);
    $('#currentSlide').html((currSpecSlide+1) + " of " + listCount);
   
    setTimeout(function(){
        $('#'+currSlide + '>a').trigger('click');	
        sscResizeWindow();
    },10);
    $('#prev_bn').attr('disabled','disabled').addClass('prevbtn_disable').removeClass('prevbtn_enable').css('opacity','1');
}
...
function bindEvents(){
    ...
    $('.navDrop').click(function(e){
        ...
        $('#nav>li>a').css('background','#444444').css('color','#ffffff');
     	$('#nav>li>ul>li>a').css('background','#222527').css('color','#ffffff');
     		$('#nav li a').removeClass('clk');
		$('#'+currSlide +">a").css('background','#262626').css('color','#fff').addClass('clk');
		loadAct(screenUrl);
		e.preventDefault(); 
		e.stopPropagation();
		navClicked = "1";
		...
		if (parseInt(currSpecSlide)+1 == parseInt(listCount)){
		    $('#next_bn').attr('disabled','disabled').addClass('nextbtn_disable').removeClass('nextbtn_enable').css('opacity','1');
		}else{
	...
	$('#next_bn').click(function(e){
        ...
        if (parseInt(currSpecSlide) < parseInt(listCount)){
            ...
            loadAct(screenUrl);
     //       $('#currentSlide').html(listArray[currSpecSlide].value + " of " + listCount);
	//        $('#currentSlide').html(listArray[currSpecSlide].value + " of " + menuCount);
       ...
       $('#currentSlide').html((currSpecSlide+1) + " of " +listCount);
			...
			chkBeads(currSlide,beedle);
			$('#nav>li>a').css('background','#444444').css('color','#babbbb');
